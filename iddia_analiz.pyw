...
class mainWindow(tk.Tk):
    def __init__(self):
        ...
        self.dinamik_etiket_toplammac = tk.Label(self, text="Toplam bulunan maç:", font=("Helvetica", 15))
        self.dinamik_ortalama_gol = tk.Label(self, text="Ortalama gol sayısı:", font=("Helvetica", 15))
        ...
        self.dinamik_etiket_toplammac.grid(row=5,column=0,columnspan=2)
        self.dinamik_ortalama_gol.grid(row=6,column=0,columnspan=2)
    ...
    def arama_yap(self, *args):
        ...
        gereksiz_liste = list(self.c.fetchall())
        ...
        if len(gereksiz_liste) > 0:
            ustoran = ust / (len(gereksiz_liste) - toplama_katilmayacak_mac_sayisi)
            altoran = alt / (len(gereksiz_liste) - toplama_katilmayacak_mac_sayisi)
            toplamgoloran = toplamgol / (len(gereksiz_liste) - toplama_katilmayacak_mac_sayisi)
            self.dinamik_etiket_alt.configure(text="Üst Bitme Oranı: % {}".format(round(altoran*100, 2)))
            self.dinamik_etiket_ust.configure(text="Alt Bitme Oranı: % {}".format(round(ustoran*100, 2)))
            self.dinamik_etiket_toplammac.configure(text="Toplam bulunan maç: {}".format(len(gereksiz_liste)))
            self.dinamik_ortalama_gol.configure(text="Ortalama gol sayısı: {}".format(round(toplamgoloran, 2)))
            ...
            self.dinamik_etiket_toplammac.configure(text="Toplam bulunan maç: 0")
            self.dinamik_ortalama_gol.configure(text="Ortalama gol sayısı:")
